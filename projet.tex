...
It has a bottleneck of size ??? and reconstructs the images with an average per-pixel error of $???$.
\todo{add error rate}
...
\paragraph{}
Passing the adversarial images from \autoref{fig:dataset_sizes},
...
\begin{figure}[h]
  ...
  \caption{Examples of the autoencoder defense.}
  \label{fig:ae_examples}
\end{figure}

Passing the image through the VAE also recovers most of the accuracy
of the classifiers, as seen in \autoref{table:ae_examples_accuracy}.
...
\paragraph{Attacking the composition of an MNIST autoencoder and classifier}

\paragraph{Attacking the composition of ImageNet autoencoder and classifier}

\subsection{Attacking the autoencoder}
\paragraph{Attacking the ImageNet autoencoder to produce a very different image}
